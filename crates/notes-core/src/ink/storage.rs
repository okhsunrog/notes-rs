...
pub(super) fn open(store: &Store) -> CommandResult<Connection> {
    ...
    conn.execute_batch("PRAGMA foreign_keys=ON; PRAGMA synchronous=FULL;")
        .map_err(err)?;
    ensure_document(&conn, store.document)?;
    conn.execute(
...
pub(super) fn ensure_document(conn: &Connection, document: uuid::Uuid) -> CommandResult<()> {
    ...
    match kind.as_deref() {
        Some("ink") => Ok(()),
        Some(_) => Err(CommandError::invalid("Not a handwritten note")),
        None => Err(CommandError::not_found(
            "Handwritten note was deleted or does not exist",
        )),
    }
}
...
pub(super) fn read(path: &Store) -> CommandResult<InkDraftSnapshot> {
    ...
    let result = match load(&tx, path.document)? {
...
pub(super) fn patch(
    ...
    if current.is_none() {
        ...
        tx.execute(
            "INSERT INTO ink_history VALUES(?2,0,?1)",
            params![initial.root.id.as_slice(), path.document],
        )
        .map_err(err)?;
    }
    ...
    tx.execute(
        "DELETE FROM ink_history WHERE page_uuid=?2 AND seq>?1",
        params![cursor, path.document],
    )
    .map_err(err)?;
    tx.execute(
        "INSERT INTO ink_history VALUES(?3,?1,?2)",
        params![cursor + 1, snapshot.root.id.as_slice(), path.document],
    )
    .map_err(err)?;
    let revision = history::set_head(&tx, path.document, cursor + 1, snapshot.root.id)?;
    tx.execute(
        "DELETE FROM ink_history WHERE page_uuid=?2 AND seq<?1",
        params![cursor + 1 - 50, path.document],
    )
    .map_err(err)?;
    history::collect(&tx)?;
    tx.commit().map_err(err)?;
