//! Regression coverage moved from the removed singleton Tauri adapter.
use super::*;
fn new_store(path: std::path::PathBuf) -> Store {
    ...
    tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async {
            ...
            let page = crate::db::create_handwritten_note_with_ops(&conn, None)
                .await
                .unwrap()
                .value;
            Store::new(path, page.uuid)
        })
}
fn read_draft(store: &Store) -> CommandResult<InkDraftSnapshot> {
    store.read()
}
fn write_patch(store: &Store, patch: InkDraftPatch, rev: Option<String>) -> CommandResult<String> {
    store.patch(patch, rev)
}
fn write_draft(store: &Store, draft: InkDraft, rev: Option<String>) -> CommandResult<String> {
    validate(&draft)?;
    store.patch(
        InkDraftPatch {
...
mod storage {
    use super::*;
    pub fn patch(s: &Store, p: InkDraftPatch, r: Option<String>) -> CommandResult<String> {
        s.patch(p, r)
    }
    pub fn compact(s: &Store) -> CommandResult<bool> {
        s.compact()
    }
    pub fn navigate_update(
        s: &Store,
        redo: Option<bool>,
        r: Option<String>,
    ...
    pub fn navigate(
        ...
        s.history(redo, r)?;
        let InkHistoryUpdate::Snapshot { history } = s.history(None, None)? else {
            unreachable!()
        };
        Ok(history)
    }
...
fn patches_preserve_unchanged_points_delete_and_restore_strokes() {
    ...
    let path = new_store(dir.path().join("ink.sqlite3"));
...
fn preserves_strokes_and_rejects_stale_writes() {
    let dir = tempfile::tempdir().unwrap();
    let path = new_store(dir.path().join("ink/ink.sqlite3"));
...
fn invalid_data_never_replaces_the_last_saved_draft() {
    ...
    std::fs::write(&path.path, b"broken file").unwrap();
    assert!(write_draft(&path, InkDraft::default(), None).is_err());
    assert_eq!(std::fs::read(&path.path).unwrap(), b"broken file");
}
fn stored_rows(path: &Store, table: &str) -> Vec<(Vec<u8>, Vec<u8>)> {
...
fn failure_during_head_update_rolls_back_every_new_blob() {
    ...
    {
        let conn = rusqlite::Connection::open(&path.path).unwrap();
        conn.execute_batch("CREATE TRIGGER fail_commit BEFORE UPDATE ON ink_documents BEGIN SELECT RAISE(ABORT,'injected failure'); END;").unwrap();
    }
