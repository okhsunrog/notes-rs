...
Уточнение пользователя от 2026-09-06: рабочее хранение остаётся в SQLite, с отдельными
CBOR metadata records и INKCHNK BLOB, связанными корнем документа. Одна заметка не
обязана быть одним BLOB. Самодостаточный файл для импорта/экспорта целой заметки —
отложенная задача; имя, расширение и устройство контейнера пока не выбираем.
Рабочее имя INKDOC не закреплено. Контейнер не блокирует общую базу и синхронизацию;
переход с SQLite на файлы не планируется. Независимость крейта от способа хранения
сохраняем без реализации второго backend. Подробности: `handwriting-integration.md`.
